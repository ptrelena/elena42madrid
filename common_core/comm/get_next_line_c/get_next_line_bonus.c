...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0) //failure check
		return (NULL);
	//[fd] is used to indicate what fd the ft is working with in each call
	st_buff[fd] = ft_read_and_join(fd, st_buff[fd]); //start reading + join read bytes
	if (!st_buff[fd]) //fd failure check
		return (NULL);
	if (!*st_buff[fd]) //fd array failure check
		return (free(st_buff[fd]), st_buff[fd] = NULL, NULL);
	line_found = ft_find_newline(st_buff[fd]); //look for newline
	st_buff[fd] = ft_latest_buffer(st_buff[fd]); //check buff beggining each gnl call
